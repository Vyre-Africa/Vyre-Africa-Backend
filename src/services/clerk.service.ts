...
class ClerkService{
    ...
    async processEvent(evt: any) {
        const { id, type: eventType, ...data } = evt;

        try {
            logger.info('Processing webhook event', { eventId: id, eventType });
    ...
    private async handleUserCreated(userData: any) {
        ...
        logger.info('Handling user.created', { email: emailAddress, clerkId: userData.id });
        ...
        if (existingUser) {
            ...
            logger.info('Existing user updated with Clerk data', { 
                id: userData.id, 
                clerkId: userData.id 
            });
        } else {
    ...
    private async handleUserUpdated(userData: any) {
        ...
        logger.info('Handling user.updated', { email: emailAddress, clerkId: userData.id });
        ...
        logger.info('User updated', { userId: user.id, clerkId: userData.id });
    }

    // ✅ Handle user.deleted event (optional)
    private async handleUserDeleted(userData: any) {
        logger.info('Handling user.deleted', { clerkId: userData.id });
        ...
        if (!user) {
            logger.warn('User not found for deletion', { clerkId: userData.id });
            return;
        }
        ...
        logger.info('User deactivated', { userId: user.id, clerkId: userData.id });
    }
