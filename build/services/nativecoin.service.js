...
class CoinConfigurations {
    ...
    static isSupported(coin) {
        return coin in this.CONFIGS;
    }
    static isCrypto(coin) {
        return !!this.CONFIGS[coin]?.webhookChain;
    }
}
CoinConfigurations.CONFIGS = {
    BTC: {
        tatumCurrency: 'BTC',
...
class AuthenticationHelper {
    static getAuthConfig(coin, index) {
    ...
    static getMnemonicAuth(coin, index) {
        const mnemonicMap = {
            BTC: env_config_1.default.BTC_MNEMONIC || '',
            ETH: env_config_1.default.ETH_MNEMONIC || '',
            LTC: env_config_1.default.LTC_MNEMONIC || '',
            TRON: env_config_1.default.TRON_MNEMONIC || ''
        };
        const xpubMap = {
            BTC: env_config_1.default.BTC_XPUB || '',
            LTC: env_config_1.default.LTC_XPUB || ''
        };
        const auth = {
    ...
    static getPrivateKeyAuth(coin) {
        if (coin === 'BNB') {
            return { fromPrivateKey: env_config_1.default.BNB_KEY };
        }
        throw new Error(`Private key auth not configured for ${coin}`);
    }
    static getSecretAuth(coin) {
        if (coin === 'XRP') {
            return {
                account: env_config_1.default.XRP_ADDRESS,
                secret: env_config_1.default.XRP_SECRET
            };
        }
...
class nativeCoinService {
    ...
    // ============================================
    async createWallet(coin, userId, currencyId) {
        try {
            ...
            logger_1.default.info(`Creating ${coin} wallet`, { userId, currencyId });
            ...
            if (coin === 'BTC')
                accountData.xpub = env_config_1.default.BTC_XPUB;
            if (coin === 'ETH')
                accountData.xpub = env_config_1.default.ETH_XPUB;
            if (coin === 'LTC')
                accountData.xpub = env_config_1.default.LTC_XPUB;
            if (coin === 'TRON')
                accountData.xpub = env_config_1.default.TRON_XPUB;
            if (coin === 'BNB')
                accountData.xpub = env_config_1.default.BNB_ADDRESS;
            if (coin === 'XRP')
                accountData.xpub = env_config_1.default.XRP_ADDRESS;
            ...
            if (isCrypto) {
                ...
                if (destinationTag)
                    walletData.destinationTag = destinationTag;
            }
            const wallet = await prisma_config_1.default.wallet.create({ data: walletData });
            logger_1.default.info(`${coin} wallet created successfully`, {
                walletId: wallet.id,
            ...
            logger_1.default.error(`Failed to create ${coin} wallet:`, error);
            throw error;
        }
    }
    // ============================================
    ...
    // ============================================
    async blockchainTransfer(coin, payload) {
        try {
            ...
            const coinConfig = CoinConfigurations.getConfig(coin);
            logger_1.default.info(`Initiating ${coin} transfer`, {
                userId,
                walletId,
            ...
            if (availableBalance.lessThan(amountDecimal)) {
                throw new Error(`Insufficient balance. Available: ${availableBalance.toFixed(8)} ${coin}, Required: ${amountDecimal.toFixed(8)} ${coin}`);
            }
            // Build transfer data
            const transferData = {
            ...
            logger_1.default.info('Executing blockchain transfer', {
                coin,
            ...
            if (!result.completed) {
                logger_1.default.info('Completing pending withdrawal', {
                    withdrawalId: result.id,
                    txId: result.txId
                });
                transaction = await this.completeWithdrawal(result.id, result.txId);
            }
            logger_1.default.info(`${coin} transfer completed successfully`, {
            ...
            logger_1.default.error(`${coin} transfer failed:`, {
                error: error.message,
                coin,
    ...
    async createWalletByISO(ISO, userId, currencyId) {
        if (!CoinConfigurations.isSupported(ISO)) {
            throw new Error(`Currency ${ISO} not supported`);
        }
        return this.createWallet(ISO, userId, currencyId);
    }
    ...
    async blockchain_Transfer(payload) {
        ...
        return this.blockchainTransfer(ISO, {
            userId,
            walletId,
            address,
            amount,
...
//     }
// }
exports.default = new nativeCoinService();
