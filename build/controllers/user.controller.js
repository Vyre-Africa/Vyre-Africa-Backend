...
class UserController {
    async generatePin(req, res) {
        ...
        try {
            ...
            logger_1.default.info('🔵 PIN generation requested', { email, phoneNumber });
            // ============================================
            // CHECK IF USER EXISTS
            // ============================================
            const existingUser = await prisma_config_1.default.user.findFirst({
                where: {
                    email,
            ...
            // GENERATE NEW PIN
            // ============================================
            const newPin = (0, utils_1.generateAccessPin)();
            const hashedPin = await (0, utils_1.hashPin)(newPin);
            // ✅ ONLY TEMP USERS GET EXPIRY
            const expiresAt = existingUser
                ? null
            ...
            if (existingUser) {
                ...
                try {
                    ...
                    logger_1.default.info('✅ Reusable PIN regenerated for existing user', { userId: user.id });
                }
                catch (updateError) {
                    logger_1.default.error('❌ Failed to update user PIN', {
                ...
                user = await prisma_config_1.default.tempUser.upsert({
                    where: {
                ...
                logger_1.default.info('✅ Temporary PIN upserted', {
                    tempUserId: user.id
                });
            }
            ...
            setImmediate(async () => {
                try {
                    ...
                    logger_1.default.info('✅ PIN notification queued successfully', {
                        userId: user.id,
                        email: (0, utils_1.maskEmail)(email)
                    });
                }
                catch (notificationError) {
                    logger_1.default.error('❌ Failed to queue PIN notification (non-critical)', {
                        userId: user.id,
                        email: (0, utils_1.maskEmail)(email),
            ...
            logger_1.default.error('🔴 PIN generation failed', {
                email,
                error: error.message,
                stack: error.stack
            });
    ...
    async verifyPin(req, res) {
        try {
            ...
            logger_1.default.info('🔍 PIN verification attempt', {
                email,
                phoneNumber
            });
            // ============================================
            ...
            // STEP 6: VERIFY PIN
            // ============================================
            const isValid = await (0, utils_1.verifyPin)(pin, userRecord.accessPin);
            if (isValid) {
                // ✅ PIN IS CORRECT
                ...
                if (existingUser) {
                    ...
                    }).catch(err => logger_1.default.error('Failed to reset PIN attempts', err));
                }
                logger_1.default.info('✅ PIN verification successful', {
                    email,
                ...
                if (existingUser) {
                    ...
                    }).catch(err => logger_1.default.error('Failed to update PIN attempts', err));
                    const attemptsRemaining = 5 - attempts;
                    logger_1.default.warn('❌ PIN verification failed', {
                        email,
                        attempts,
                        shouldLock
                    });
                    ...
                    logger_1.default.warn('❌ PIN verification failed for temp user', { email });
                    return res.status(400).json({
                        success: false,
                        valid: false,
            ...
            logger_1.default.error('🔴 PIN verification error', {
                error: error.message,
                stack: error.stack
            });
            return res.status(500).json({
    ...
    async sendAccessPinEmail(params) {
    ...
    async register(req, res) {
        ...
        const { sub, given_name, family_name, picture, email, email_verified } = req.user;
        // console.log(req.body)
        try {
            // let referree:any;
            const userExist = await prisma_config_1.default.user.findUnique({
                where: {
                    authId: sub,
                    email
                },
            ...
            const result = await prisma_config_1.default.$transaction(async (prisma) => {
    ...
    async register_Kyc(req, res) {
        const user = req.user;
        const { legalFirstName, legalLastName, phoneNumber, dateOfBirth, nationalIdType, nationalIdNumber, employmentStatus, mostRecentOccupation, sourceOfFunds, accountPurpose, expectedMonthlyPaymentsUsd, address, documents } = req.body;
        // console.log(req.body)
        try {
            ...
            const result = await prisma_config_1.default.$transaction(async (prisma) => {
                const customer = await fern_service_1.default.customer({
                    legalFirstName,
                    legalLastName,
                    phoneNumber,
                    nationalIdType,
    ...
    async setPassword(req, res) {
        ...
        try {
            ...
            const encryptedPassword = await (0, utils_1.hashData)(password);
            await prisma_config_1.default.user.update({
                where: { id: user.id },
                data: {
    ...
    async resendOtpCode(req, res) {
        const { email } = req.body;
        const user = await prisma_config_1.default.user.findUnique({ where: { email: email } });
        if (!user) {
            return res.status(400).json({
        ...
        // moment().isAfter(user?.otpCodeExpiryTime)
            ...
            await prisma_config_1.default.user.update({
                ...
                data: {
                    otpCode: otpCode,
                    otpCodeUsed: false,
                    otpCodeExpiryTime: utils_1.OTP_CODE_EXP,
                },
            });
            await mail_service_1.default.sendOtp(user?.email, user.firstName, otpCode);
            // await mailService.sendMail(user?.email,otpCode)
        }
        else {
            await mail_service_1.default.sendOtp(user?.email, user.firstName, user.otpCode);
    ...
    async sendVerification(req, res) {
        ...
        try {
            const EmailSent = await mail_service_1.default.sendMail(email, OTP);
    ...
    async loginUser(req, res) {
        ...
        try {
            ...
            const pwdCorrect = await (0, utils_1.compareHashedData)(password, user.password);
    ...
    async authenticateViaOtp(req, res) {
    ...
    async getProfile(req, res) {
        ...
        try {
            ...
            const { hasAnyPaymentMethod, hasFiatAccount, hasCryptoAccount } = await (0, utils_1.checkUserPaymentMethods)(userData.id);
            return res.status(201).json({
    ...
    async changePassword(req, res) {
        ...
        try {
            ...
            if (!user.password || !await (0, utils_1.compareHashedData)(currentPassword, user.password)) {
                return res.status(400).json({
                    msg: 'current password is incorrect',
                    success: false,
                });
            }
            const encryptedPassword = await (0, utils_1.hashData)(newPassword);
            await prisma_config_1.default.user.update({
                where: { email: user.email },
                data: {
                    password: encryptedPassword
    ...
    async getAllBanks(req, res) {
        ...
        console.log('got in ');
        banks = await prisma_config_1.default.bank.findMany({
            where,
            take: Number(limit),
            select: {
                id: true,
    ...
    async getCurrencyBanks(req, res) {
        // const banks = await paystackService.getBanks();
    ...
    async queryUser(req, res) {
        ...
        console.log('got in ');
        users = await prisma_config_1.default.user.findMany({
    ...
    async verifyAccountDetail(req, res) {
        ...
        const verifyDetails = await flutterwave_service_1.default.resolveAccount(bank.code, accountNumber);
    ...
    async addBank(req, res) {
        ...
        try {
            ...
            let userBank = await prisma_config_1.default.userBank.findFirst({
                where: {
                    userId: user.id,
                    accountNumber
                }
            ...
            userBank = await prisma_config_1.default.userBank.create({
                data: {
                    accountName,
                    accountNumber,
                    userId: user.id,
    ...
    async getUserBank(req, res) {
        const user = req.user;
        const userBanks = await prisma_config_1.default.userBank.findMany({
            where: {
                userId: user.id,
            },
            include: {
    ...
    async deleteUserBank(req, res) {
        const user = req.user;
        const userBankId = req.params.userBankId;
        try {
            let userBank = await prisma_config_1.default.userBank.findUnique({
                where: { id: userBankId }
            });
            if (!userBank) {
                return res.status(400).json({
            ...
            await prisma_config_1.default.userBank.delete({
                where: { id: userBankId }
            });
            const userBanks = await prisma_config_1.default.userBank.findMany({
                where: {
    ...
    async setNotificationMethod(req, res) {
        const user = req.user;
        const { emailNotification, pushNotification, smsNotification } = req.body;
        try {
            let notificationSetting = await prisma_config_1.default.notificationSetting.findUnique({
                where: { userId: user.id }
            });
            if (notificationSetting) {
                notificationSetting = await prisma_config_1.default.notificationSetting.update({
                    where: { userId: user.id },
                    data: {
                        emailNotification,
                        pushNotification,
                ...
                notificationSetting = await prisma_config_1.default.notificationSetting.create({
                    data: {
                        userId: user.id,
                        emailNotification,
                        pushNotification,
    ...
    async getNotificationMethod(req, res) {
        const user = req.user;
        const notificationSetting = await prisma_config_1.default.notificationSetting.findUnique({
            where: { userId: user.id }
        });
        if (!notificationSetting) {
            const newSetting = await prisma_config_1.default.notificationSetting.create({
                data: {
                    userId: user.id,
    ...
    async setTwoFactorAuthenticationMethod(req, res) {
        const user = req.user;
    ...
    async disableTwoFactorAuthenticationMethod(req, res) {
        const user = req.user;
    ...
    async getTwoFactorAuthenticationMethod(req, res) {
        const user = req.user;
        const userData = await prisma_config_1.default.user.findUnique({
            where: { id: user.id }
        });
    ...
    async getNotification(req, res) {
        ...
        try {
            const notifications = await notification_service_1.default.getUserNotification(user.id, limit);
            return res.status(201).json({
    ...
    async filterNotification(req, res) {
        ...
        try {
            ...
            const notifications = await notification_service_1.default.filterUserNotification(user.id, limit, dateFrom, dateTo, type);
            return res.status(201).json({
                msg: 'Notifications fetched successfully',
                success: true,
                notifications
    ...
    async getTransactionsByStatus(req, res) {
        ...
        try {
            ...
            const transactions = await transaction_service_1.default.filterByStatus(user.id, limit, status);
            return res.status(201).json({
                msg: 'Transactions fetched successfully',
                success: true,
                transactions
    ...
    async getTransactionsByType(req, res) {
        ...
        try {
            ...
            const transactions = await transaction_service_1.default.filterByType(user.id, limit, payment);
            return res.status(201).json({
                msg: 'Transactions fetched successfully',
                success: true,
                transactions
    ...
    async fundWallet(req, res) {
        ...
        try {
            ...
            // )
            //save notification
            await notification_service_1.default.create(user.id, null, 'Wallet Funding', 'You funded your wallet with ' + amount, 'TOPUP');
